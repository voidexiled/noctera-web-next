...
export const BattlepassTask = ({ task, playerTask }: BattlepassTaskType) => {
	...
	// 	);
	return (
		<div className="group no-draggable no-selectable relative mr-4 grid h-[140px] w-[150px] grid-cols-1 grid-rows-[20px_1fr] overflow-hidden rounded-md bg-secondary/40 shadow-xs transition-all duration-300 hover:bg-secondary/60">
			...
			<div className="relative grid h-full w-full grid-cols-1 grid-rows-[1fr_35px] items-center justify-start px-2">
				...
				<span className="text-center text-secondary-foreground/90 text-xs">{task.task_name}</span>
			</div>
		</div>
	);
};
