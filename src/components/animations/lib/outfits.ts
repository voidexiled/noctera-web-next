...
import { mounts, outfitColors } from "./config";
...
export function loadData(
	...
	const outfitDataPath = path.join(outfitPath, outfitId.toString(), "outfit.data.txt");
	if (!fs.existsSync(outfitDataPath)) {
		throw new Error(`Outfit data not found: ${outfitDataPath}`);
	}

	const rawData = fs.readFileSync(outfitDataPath);

	const deserializedData = unserialize(rawData);
	const jsonData = JSON.stringify(deserializedData, null, 2);

	const tmp = JSON.parse(jsonData) as OutfitData;
...
function fileExists(filePath: string, outfitData: OutfitData): boolean {
	const normalizedFilePath = filePath.replace(/\\/g, "/").replace(/^\.\//, "").replace(/\/$/, "");
...
export async function outfit(
	...
	templateCanvas && colorize(templateCanvas, outfitCanvas, head, body, legs, feet);
	...
	const imageOutfitTCanvas = createCanvas(resizeTo64px ? 64 : width, resizeTo64px ? 64 : height);
...
async function colorize(
	...
	const outfitImageData = outfitContext.getImageData(0, 0, outfitCanvas.width, outfitCanvas.height);
	const outfitPixels = outfitImageData.data;

	for (let i = 0; i < templatePixels.length; i += 4) {
		const templatePixel =
			(templatePixels[i] << 16) | (templatePixels[i + 1] << 8) | templatePixels[i + 2];
		const outfitPixel = (outfitPixels[i] << 16) | (outfitPixels[i + 1] << 8) | outfitPixels[i + 2];
