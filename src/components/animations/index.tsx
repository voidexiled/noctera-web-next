...
const OutfitComponent: React.FC<OutfitComponentProps> = ({
	...
	return (
		<div className={`relative h-12 w-12 ${klass} overflow-visible`}>
			<div
				className={`absolute ${
					hasMount ? "-left-7 -bottom-1" : "-left-10 bottom-1"
				} ${innerClass}`}
			>
				{frames && outfit.looktype > 0 ? (
					<canvas ref={canvasRef} className="h-20 w-20" aria-details={alt} />
				) : (
