		...
		try {
			...
			if (response.ok) {
				toast({
					...
					description: <div>{data.amount} add.</div>,
				});
				setOpen(false);
				reset();
				return;
			...
			toast({
				title: "Error",
				variant: "destructive",
				description: <div>Add coin as been failed.</div>,
			});
		} catch (error) {
			reset();
		}
						...
						<div className="grid gap-4 py-2">
							<div className="grid gap-2 space-y-2">
								...
								<RHFSelect
									...
									label="type"
									options={[
										{ value: "coins_transferable", label: "Transferable" },
										{ value: "coins", label: "No transferable" },
									]}
									name="type"
									defaultValue={"coins"}
									disabled={isSubmitting}
