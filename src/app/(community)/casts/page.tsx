...
const PlayersCasts = async ({
	...
	const count = await prisma.players.count({
		where: { cast_on: 1, name: { contains: search ? search : undefined } },
	});
	...
	return {
		players: convertBigIntsToNumbers(query),
		totalPage: Math.ceil(Number(count) / ITEMS_PER_PAGE),
	};
};
								...
								{players.map((player: players, index: number) => {
									return (
										<TableRow key={player.id} className="text-xs">
											...
											<TableCell className="w-[180px] items-start justify-start space-y-2">
												...
												<div className="flex flex-row items-end gap-2 whitespace-nowrap">
													<b>[Bonus 10%] </b>{" "}
													<picture>
														{" "}
														<img src="/icons/XP_Boost_Icon.gif" alt="" />{" "}
													</picture>
												</div>
											</TableCell>
										</TableRow>
					...
					<div className="rounded border">
						<Table>
							...
							<TableBody>
								...
								<TableRow className="text-xs">
									<TableCell>
										<b>Reward by using the cast with no password</b>
									</TableCell>
