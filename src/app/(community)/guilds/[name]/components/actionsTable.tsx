...
export default function TableActions({
	...
	async function KickPlayer(guild_id: number, player_id: number) {
		...
		const res = await fetch(
			`/api/guilds/manager/${guild_id}/kick/${player_id}`,
			{ method: "DELETE" },
		);
	...
	async function UpdatePlayerRank(
		...
		// const res = await fetch(`/api/guilds/manager/${guild_id}/ranks`, { method: 'PUT', body: JSON.stringify(formData) })
		const res = await fetch(
			`/api/guilds/manager/${guild_id}/player/${player_id}`,
			{ method: "PUT", body: JSON.stringify({ rank_id }) },
		);
		...
		<>
			<DropdownMenu>
				...
				<DropdownMenuContent>
									...
									>
										{ranks.map((rank) => {
											return (
												...
												>
													<IconiFy
														icon={`game-icons:rank-${rank.level}`}
														className="h-8 w-8"
													/>{" "}
													{rank.name}
												</DropdownMenuRadioItem>
					...
					<DropdownMenuItem
						onClick={() => KickPlayer(row.guild_id, row.player_id)}
					>
						Kick
					</DropdownMenuItem>
