...
import { StepFormContext } from "@/app/(webshop)/shop/components/StepFormContext";
import { formatCurrency } from "@/app/(webshop)/shop/utils/functions";
...
import type { accounts, orders } from "@prisma/client";
...
export function Payment({ paymentIntent, account }: PaymentProps) {
	...
	async function confirmStripePayment() {
		startTransition(async () => {
			...
			// ORDER -> PROCESSING
			toast.promise(
				fetch("/api/shop/orders/update_status", {
					method: "POST",
					headers: {
		...
		<>
			...
			<div className="mt-3 text-pretty border border-rose-500 bg-rose-500/30 p-3 text-xs ">
				...
				<p className="text-pretty text-sm">
					...
					<Link href="/return-policy" className="text-blue-500">
						Return Policy
					</Link>{" "}
					and{" "}
