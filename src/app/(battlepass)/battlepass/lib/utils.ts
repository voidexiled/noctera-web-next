...
export const isDateActive = (
	...
	console.log("CONVERSION START DATE / END DATE A UNIX:");
...
export function getTaskPath(
	task_type: BATTLEPASS_TYPE_TASKS,
	task_image: string,
) {
	...
	basePath = basePath.concat(
		task_image !== "" ? task_image.toString() : "0.gif",
	);

	return basePath;
}

export function getRewardPath(
...
export const getDisplayRankTextClassname = (
	rank:
		| (typeof BATTLEPASS_RANK_ACCESS)[keyof typeof BATTLEPASS_RANK_ACCESS]
		| undefined,
) => {
	if (rank === BATTLEPASS_RANK_ACCESS.FREE) return "text-battlepass-rank-free";
	if (rank === BATTLEPASS_RANK_ACCESS.VIP_SILVER)
		return "text-battlepass-rank-vip-silver ";
	if (rank === BATTLEPASS_RANK_ACCESS.VIP_GOLD)
		return "text-battlepass-rank-vip-gold ";
	if (rank === BATTLEPASS_RANK_ACCESS.DIAMOND)
		return "text-battlepass-rank-diamond ";
	return "";
};
...
export const formatTibianMoney = (amount: number): string => {
	const k = 1000;
	const kk = k * k;
	const kkk = k * kk;
...
export const getRewardSuccessfullyClaimedText = (
	battlepass_reward: battlepass_seasons_rewards,
) => {
...
export const calculateCurrentItemRemainingRewards = (
	...
	const battlepassLevelRewardsIds = battlepass_level.rewards.map(
		(reward) => reward.id,
	);
	const totalRewards = battlepass_level.rewards.filter((reward) => {
		return hasAccessToReward(
			player.battlepass_rank,
			reward.reward_required_access,
		);
	});

	const claimedRewards = player.player_battlepass_rewards_claimed.filter(
		(reward) => battlepassLevelRewardsIds.includes(reward.reward_id),
	);
