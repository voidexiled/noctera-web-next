"use client";
import { Payment } from "@/app/(default)/(webshop)/donate/components/Payment";
import { ShopContext } from "@/app/(default)/(webshop)/donate/components/ShopContext";
import { Step, StepFormContext } from "@/app/(default)/(webshop)/donate/components/StepFormContext";
		...
		toast
			.promise(
				typedFetch<
					ShopStripePaymentIntentsCreatePOSTRequest,
					ShopStripePaymentIntentsCreatePOSTResponse
				>(API_ROUTES.shop.stripe.paymentIntents.create._, {
					method: "POST",
					body: {
			...
			.unwrap()
			.then((res) => {
				console.log("updateing clientSecret", res.paymentIntent.client_secret);
